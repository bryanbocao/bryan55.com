<?php
    ...
    // increase database record
    $connection = mysql_connect("localhost","bryan55","bryan55201402171747");
    $db = "bryan55";
    mysql_select_db($db, $connection) or die("fail");//'fail' means cannot connect the database
    date_default_timezone_set("PRC");
    $viewingDate = date("Y-m-d H:i:s");
     if($_SERVER["REMOTE_ADDR"]){// only record ip address if it can be got by "REMOTE_ADDR"
        $ip = $_SERVER["REMOTE_ADDR"];
        $sqlInsertRecord = "INSERT INTO bryan55_website_demos_holidaysystem (ip, viewing_date) VALUES ('{$ip}','{$viewingDate}')";
        // insert ip records
        mysql_query($sqlInsertRecord,$connection);
    }
